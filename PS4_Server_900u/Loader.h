...
function injectPayload() //dynamic payload inject - stooged
{
    if (payloadData.length > 0)
    {
       ...
       showMessage(payloadTitle + " Загружено.");
    }
  else
  {
...
function run_hax() {
    ...
    {
       showMessage("Загрузка " + payloadTitle + "...");
...
var trigger_spray =  function () {
    ...
    showMessage("Загрузка USB, ExFatHax...");
    enableUSB();
...
<div class="info" id="message">Загрузка эксплойта, пожалуйста, подождите...</div>
</body>
</html>
)==";
