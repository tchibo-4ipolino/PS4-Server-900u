...
int usbPin = 13;
File upFile;
String firmwareVer = "1.00 Russian version by tChibo";
...
{
    String tmphtm = "<!DOCTYPE html><html lang=\"ru-RU\"><head><style>body { background-color: #1451AE;color: #ffffff;font-size: 14px; font-weight: bold; margin: 0 0 0 0.0; padding: 0.4em 0.4em 0.4em 2.5em;}</style></head><center><br><br><br><br><br><br>" + htmMsg + "</center></html>";
...
void handleFwUpdate() {
  ...
  if (upload.filename != "fwupdate.bin") {
    sendwebmsg("Неверный файл обновления:" + upload.filename);
    return;
  }
  if (upload.status == UPLOAD_FILE_START) {
    String filename = upload.filename;
...
void updateFw()
{
 ...
 if (updateFile) {
  ...
  digitalWrite(BUILTIN_LED, LOW);
    ...
    updateFile.close();
    sendwebmsg("Обновление не удалось<br><br>" + errorMsg(Update.getError()));
    return;
    }
  ...
  sendwebmsg("Хэш загруженного файла: " + md5Hash + "<br> Хэш установленной прошивки: " + Update.md5String() + "<br><br>Обновление завершено, перезагрузка.");
  delay(1000);
  ESP.restart();
  }
  else
  {
    ...
    sendwebmsg("Обновление не удалось");
     //Update.printError(Serial);
    }
  }
  }
  ...
  digitalWrite(BUILTIN_LED, HIGH);
  SPIFFS.remove("/fwupdate.bin");
  sendwebmsg("Ошибка, файл неверный");
  return;    
  }
  }
  }
  else
  {
    ...
    sendwebmsg("Файл обновления не найден");
  }
}
...
void handleFileMan() {
  ...
  while(dir.next()){
    ...
    output += "<td><form action=\"/" + fname + "?download=1\" method=\"post\"><button type=\"submit\" name=\"file\" value=\"" + fname + "\">Скачать</button></form></td>";
    output += "<td><form action=\"/delete\" method=\"post\"><button type=\"submit\" name=\"file\" value=\"" + fname + "\" onClick=\"return statusDel('" + fname + "');\">Удалить</button></form></td>";
    output += "</tr>";
    }
    entry.close();
  }
  if (fileCount == 0)
  {
      output += "<center><msg>Файлов не найдено<br>Вы можете загружать файлы с помощью <b>Файлового Менеджера</b></msg></center>";
  }
...
void handlePayloads() {
  Dir dir = SPIFFS.openDir("/");
  String output = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>PS4 9.00 Jailbreak ESP Сервер</title><script>function setpayload(payload,title,waittime){ sessionStorage.setItem('payload', payload); sessionStorage.setItem('title', title); sessionStorage.setItem('waittime', waittime);  window.open('loader.html', '_self');}</script><style>.btn { background-color: DodgerBlue; border: none; color: white; padding: 12px 16px; font-size: 16px; cursor: pointer; font-weight: bold;}.btn:hover { background-color: RoyalBlue;}body { background-color: #1451AE; color: #ffffff; font-size: 14px; font-weight: bold; margin: 0 0 0 0.0; overflow-y:hidden; text-shadow: 3px 2px DodgerBlue;} .main { padding: 0px 0px; position: absolute; top: 0; right: 0; bottom: 0; left: 0; overflow-y:hidden;} msg {color: #ffffff; font-weight: normal; text-shadow: none;} a {color: #ffffff; font-weight: bold;}</style></head><body><center><h1>PS4 9.00 Пейлоады</h1>";
  int cntr = 0;
  int payloadCount = 0;
  while(dir.next()){
    File entry = dir.openFile("r");
  ...
  if (payloadCount == 0)
  {
      output += "<msg>Не найдено .bin пэйлодов <br> Вам нужно загрузить полезные данные на плату ESP8266 <br> в Arduino IDE выберете <b>Tools (Инструменты)</b> &gt; <b>ESP8266 Sketch Data Upload</b><br>или<br>Используя ПК/ноутбук подключитесь к WIFI сети <b>" + AP_SSID + "</b> и перейдите ао адресу <a href=http://" + Server_IP.toString() + "/admin.html>http://" + Server_IP.toString() + "/admin.html</a> и загрузите .bin пэйлоды используя <b>Загрузку файлов</b></msg></center></body></html>";
  }
...
void handleConfig()
{
  if(webServer.hasArg("ap_ssid") && webServer.hasArg("ap_pass") && webServer.hasArg("web_ip") && webServer.hasArg("web_port") && webServer.hasArg("subnet") && webServer.hasArg("wifi_ssid") && webServer.hasArg("wifi_pass") && webServer.hasArg("usbwait")) 
  {
    ...
    String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta http-equiv=\"refresh\" content=\"8; url=/info.html\"><style type=\"text/css\">#loader {  z-index: 1;   width: 50px;   height: 50px;   margin: 0 0 0 0;   border: 6px solid #f3f3f3;   border-radius: 50%;   border-top: 6px solid #3498db;   width: 50px;   height: 50px;   -webkit-animation: spin 2s linear infinite;   animation: spin 2s linear infinite; } @-webkit-keyframes spin {  0%  {  -webkit-transform: rotate(0deg);  }  100% {  -webkit-transform: rotate(360deg); }}@keyframes spin {  0% { transform: rotate(0deg); }  100% { transform: rotate(360deg); }} body { background-color: #1451AE; color: #ffffff; font-size: 20px; font-weight: bold; margin: 0 0 0 0.0; padding: 0.4em 0.4em 0.4em 2.5em;}   #msgfmt { font-size: 16px; font-weight: normal;}#status { font-size: 16px;  font-weight: normal;}</style></head><center><br><br><br><br><br><p id=\"status\"><div id='loader'></div><br>Конфигурация сохранена<br>Перезагрузка</p></center></html>";
...
void handleReboot()
{
  ...
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta http-equiv=\"refresh\" content=\"8; url=/info.html\"><style type=\"text/css\">#loader {  z-index: 1;   width: 50px;   height: 50px;   margin: 0 0 0 0;   border: 6px solid #f3f3f3;   border-radius: 50%;   border-top: 6px solid #3498db;   width: 50px;   height: 50px;   -webkit-animation: spin 2s linear infinite;   animation: spin 2s linear infinite; } @-webkit-keyframes spin {  0%  {  -webkit-transform: rotate(0deg);  }  100% {  -webkit-transform: rotate(360deg); }}@keyframes spin {  0% { transform: rotate(0deg); }  100% { transform: rotate(360deg); }} body { background-color: #1451AE; color: #ffffff; font-size: 20px; font-weight: bold; margin: 0 0 0 0.0; padding: 0.4em 0.4em 0.4em 2.5em;}   #msgfmt { font-size: 16px; font-weight: normal;}#status { font-size: 16px;  font-weight: normal;}</style></head><center><br><br><br><br><br><p id=\"status\"><div id='loader'></div><br>Перезагрузка</p></center></html>";
  webServer.setContentLength(htmStr.length());
  webServer.send(200, "text/html", htmStr);
  delay(1000);
  ESP.restart();
...
void handleConfigHtml()
{
  ...
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>Редактор конфигурации</title><style type=\"text/css\">body {    background-color: #1451AE; color: #ffffff; font-size: 14px;  font-weight: bold;    margin: 0 0 0 0.0;    padding: 0.4em 0.4em 0.4em 2.5em;}  input[type=\"submit\"]:hover {     background: #ffffff;    color: green; }input[type=\"submit\"]:active {     outline-color: green;    color: green;    background: #ffffff; }table {    font-family: arial, sans-serif;     border-collapse: collapse;}td {border: 1px solid #dddddd;     text-align: left;    padding: 8px;}  th {border: 1px solid #dddddd; background-color:gray;    text-align: center;    padding: 8px;}</style></head><body><form action=\"/config.html\" method=\"post\"><center><table><tr><th colspan=\"2\"><center>Точка доступа</center></th></tr><tr><td>AP SSID:</td><td><input name=\"ap_ssid\" value=\"" + AP_SSID + "\"></td></tr><tr><td>AP PASSWORD:</td><td><input name=\"ap_pass\" value=\"********\"></td></tr><tr><td>AP IP:</td><td><input name=\"web_ip\" value=\"" + Server_IP.toString() + "\"></td></tr><tr><td>МАСКА ПОДСЕТИ:</td><td><input name=\"subnet\" value=\"" + Subnet_Mask.toString() + "\"></td></tr><tr><td>START AP:</td><td><input type=\"checkbox\" name=\"useap\" " + tmpUa +"></td></tr><tr><th colspan=\"2\"><center>Web Server</center></th></tr><tr><td>WEBSERVER PORT:</td><td><input name=\"web_port\" value=\"" + String(WEB_PORT) + "\"></td></tr><tr><th colspan=\"2\"><center>Wifi Connection</center></th></tr><tr><td>WIFI SSID:</td><td><input name=\"wifi_ssid\" value=\"" + WIFI_SSID + "\"></td></tr><tr><td>WIFI PASSWORD:</td><td><input name=\"wifi_pass\" value=\"********\"></td></tr><tr><td>CONNECT WIFI:</td><td><input type=\"checkbox\" name=\"usewifi\" " + tmpCw + "></tr><tr><th colspan=\"2\"><center>Auto USB Wait</center></th></tr><tr><td>ВРЕМЯ ОЖИДАНИЯ (мс):</td><td><input name=\"usbwait\" value=\"" + USB_WAIT + "\"></td></tr></table><br><input id=\"savecfg\" type=\"submit\" value=\"Сохранить конфигурацию\"></center></form></body></html>";
...
{
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>Загрузка файла</title><style type=\"text/css\">#loader {  z-index: 1;  width: 50px;  height: 50px;  margin: 0 0 0 0;  border: 6px solid #f3f3f3;  border-radius: 50%;  border-top: 6px solid #3498db;  width: 50px;  height: 50px;  -webkit-animation: spin 2s linear infinite;  animation: spin 2s linear infinite;}@-webkit-keyframes spin {  0% { -webkit-transform: rotate(0deg); }  100% { -webkit-transform: rotate(360deg); }}@keyframes spin {  0% { transform: rotate(0deg); }  100% { transform: rotate(360deg); }}body {    background-color: #1451AE; color: #ffffff; font-size: 20px;  font-weight: bold;    margin: 0 0 0 0.0;    padding: 0.4em 0.4em 0.4em 2.5em;}  input[type=\"submit\"]:hover {     background: #ffffff;    color: green; }input[type=\"submit\"]:active {     outline-color: green;    color: green;    background: #ffffff;  } input[type=\"button\"]:hover {     background: #ffffff;    color: #000000; }input[type=\"button\"]:active {     outline-color: #000000;    color: #000000;    background: #ffffff; }#selfile {  font-size: 16px;  font-weight: normal;}#status {  font-size: 16px;  font-weight: normal;}</style><script>function formatBytes(bytes) {  if(bytes == 0) return '0 Bytes';  var k = 1024,  dm = 2,  sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'],  i = Math.floor(Math.log(bytes) / Math.log(k));  return parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + ' ' + sizes[i];}function statusUpl() {  document.getElementById(\"upload\").style.display=\"none\";  document.getElementById(\"btnsel\").style.display=\"none\";  document.getElementById(\"status\").innerHTML = \"<div id='loader'></div><br>Uploading files\";}function FileSelected(e){  var strdisp = \"\";  var file = document.getElementById(\"upfiles\").files;  for (var i = 0; i < file.length; i++)  {   strdisp = strdisp + file[i].name + \" - \" + formatBytes(file[i].size) + \"<br>\";  }  document.getElementById(\"selfile\").innerHTML = strdisp;  document.getElementById(\"upload\").style.display=\"block\";}</script></head><body><center><form action=\"/upload.html\" enctype=\"multipart/form-data\" method=\"post\"><p>Загрузка файлов<br><br></p><p><input id=\"btnsel\" type=\"button\" onclick=\"document.getElementById('upfiles').click()\" value=\"Выбирете файлы\" style=\"display: block;\"><p id=\"selfile\"></p><input id=\"upfiles\" type=\"file\" name=\"fwupdate\" size=\"0\" onChange=\"FileSelected();\" style=\"width:0; height:0;\" multiple></p><div><p id=\"status\"></p><input id=\"upload\" type=\"submit\" value=\"Upload Files\" onClick=\"statusUpl();\" style=\"display: none;\"></div></form><center></body></html>";
  webServer.setContentLength(htmStr.length());
  webServer.send(200, "text/html", htmStr);
}
...
{
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>Формат хранения</title><style type=\"text/css\">#loader {  z-index: 1;  width: 50px;  height: 50px;  margin: 0 0 0 0;  border: 6px solid #f3f3f3;  border-radius: 50%;  border-top: 6px solid #3498db;  width: 50px;  height: 50px;  -webkit-animation: spin 2s linear infinite;  animation: spin 2s linear infinite;}@-webkit-keyframes spin {  0% { -webkit-transform: rotate(0deg); }  100% { -webkit-transform: rotate(360deg); }}@keyframes spin {  0% { transform: rotate(0deg); }  100% { transform: rotate(360deg); }}body {    background-color: #1451AE; color: #ffffff; font-size: 20px;  font-weight: bold;    margin: 0 0 0 0.0;    padding: 0.4em 0.4em 0.4em 2.5em;}  input[type=\"submit\"]:hover {     background: #ffffff;    color: green; }input[type=\"submit\"]:active {     outline-color: green;    color: green;    background: #ffffff; }#msgfmt { font-size: 16px;  font-weight: normal;}#status {  font-size: 16px;  font-weight: normal;}</style><script>function statusFmt() { var answer = confirm(\"Are you sure you want to format?\");  if (answer) {   document.getElementById(\"format\").style.display=\"none\";   document.getElementById(\"status\").innerHTML = \"<div id='loader'></div><br>Formatting Storage\";   return true;  }  else {   return false;  }}</script></head><body><center><form action=\"/format.html\" method=\"post\"><p>Storage Format<br><br></p><p id=\"msgfmt\">This will delete all the files on the server</p><div><p id=\"status\"></p><input id=\"format\" type=\"submit\" value=\"Format Storage\" onClick=\"return statusFmt();\" style=\"display: block;\"></div></form><center></body></html>";
  webServer.setContentLength(htmStr.length());
  webServer.send(200, "text/html", htmStr);
}
...
{
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>Панель администратора</title><style>body {    background-color: #1451AE; color: #ffffff; font-size: 14px;  font-weight: bold;    margin: 0 0 0 0.0;    padding: 0.4em 0.4em 0.4em 2.5em;}.sidenav {    width: 170px;    position: fixed;    z-index: 1;    top: 20px;    left: 10px;    background: #6495ED;    overflow-x: hidden;    padding: 8px 0;}.sidenav a {    padding: 6px 8px 6px 16px;    text-decoration: none;    font-size: 14px;    color: #ffffff;    display: block;}.sidenav a:hover {    color: #1451AE;}.main {    margin-left: 150px;     padding: 10px 10px; position: absolute;   top: 0;   right: 0; bottom: 0;  left: 0;}</style></head><body><div class=\"sidenav\"><a href=\"/index.html\" target=\"mframe\">Главная</a><a href=\"/fileman.html\" target=\"mframe\">Файловый менеджер</a><a href=\"/upload.html\" target=\"mframe\">Загрузка файлов</a><a href=\"/update.html\" target=\"mframe\">Обновление прошивки</a><a href=\"/config.html\" target=\"mframe\">Конфигурация</a><a href=\"/format.html\" target=\"mframe\">Стереть память</a><a href=\"/reboot.html\" target=\"mframe\">Перезагрузить ESP</a><a href=\"/info.html\" target=\"mframe\">ESP Информация</a></div><div class=\"main\"><iframe src=\"info.html\" name=\"mframe\" height=\"100%\" width=\"100%\" frameborder=\"0\"></iframe></div></table></body></html> ";
  webServer.setContentLength(htmStr.length());
  webServer.send(200, "text/html", htmStr);
}
...
{
  String htmStr = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>ESP Reboot</title><style type=\"text/css\">#loader {  z-index: 1;   width: 50px;   height: 50px;   margin: 0 0 0 0;   border: 6px solid #f3f3f3;   border-radius: 50%;   border-top: 6px solid #3498db;   width: 50px;   height: 50px;   -webkit-animation: spin 2s linear infinite;   animation: spin 2s linear infinite; } @-webkit-keyframes spin {  0%  {  -webkit-transform: rotate(0deg);  }  100% {  -webkit-transform: rotate(360deg); }}@keyframes spin {  0% { transform: rotate(0deg); }  100% { transform: rotate(360deg); }} body { background-color: #1451AE; color: #ffffff; font-size: 20px; font-weight: bold; margin: 0 0 0 0.0; padding: 0.4em 0.4em 0.4em 2.5em;}   input[type=\"submit\"]:hover { background: #ffffff; color: green; }input[type=\"submit\"]:active { outline-color: green; color: green; background: #ffffff; } #msgfmt { font-size: 16px; font-weight: normal;}#status { font-size: 16px;  font-weight: normal;} </style><script>function statusRbt() { var answer = confirm(\"Are you sure you want to reboot?\");  if (answer) {document.getElementById(\"reboot\").style.display=\"none\";   document.getElementById(\"status\").innerHTML = \"<div id='loader'></div><br>Rebooting ESP Board\"; return true;  }else {   return false;  }}</script></head><body><center><form action=\"/reboot.html\" method=\"post\"><p>ESP Reboot<br><br></p><p id=\"msgrbt\">This will reboot the esp board</p><div><p id=\"status\"></p><input id=\"reboot\" type=\"submit\" value=\"Перезагрузить ESP\" onClick=\"return statusRbt();\" style=\"display: block;\"></div></form><center></body></html>";
  webServer.setContentLength(htmStr.length());
  webServer.send(200, "text/html", htmStr);
}
...
void handleInfo()
{
  ...
  String output = "<!DOCTYPE html><html lang=\"ru-RU\"><head><meta name = \"viewport\" content = \"width = device-width, initial-scale = 1.0\" charset=\"utf-8\"> <title>Информация о системе</title><style type=\"text/css\">body { background-color: #1451AE;color: #ffffff;font-size: 14px;font-weight: bold; margin: 0 0 0 0.0; padding: 0.4em 0.4em 0.4em 2.5em;}</style></head>";
  output += "<hr>>>>>> Програмное обеспечение <<<<<< <br><br>";
  output += "Версия прошивки " + firmwareVer + "<br>";
  ...
  output += ">>>>> CPU ######<br><br>";
  output += "CPU частота: " + String(ESP.getCpuFreqMHz()) + "MHz<br><hr>";
  ...
  output += "Flash частота: " + String(flashFreq) + " MHz<br>";
  ...
  output += ">>>>> Информация о скетче <<<<<<<br><br>";
  ...
  output += ">>>>>> Питание <<<<<<<br><br>";
  output += "Напряжение питания: " +  String(supplyVoltage) + " В<br><hr>";
  output += "</html>";
  webServer.setContentLength(output.length());
  webServer.send(200, "text/html", output);
}
